...
// An store parameters for each model
...
//nD: number of database sequences
int	nD, nW;
...
// static void read_models(string fn_prefix, int model_count) {
...
// j for database sequence index
{
	...
	for(int w = 0; w < nW; w++)
		rho[0] += Blosum62[dbAA[j][R+w]][dbAA[j][R+w]];
